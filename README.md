# Timescale Data Processing

WebAPI-приложение для загрузки и обработки временных рядов из CSV-файлов. Приложение проверяет входные данные, рассчитывает интегральную статистику, сохраняет результаты в PostgreSQL и предоставляет методы для поиска результатов и получения последних значений.

В репозитории также находится Angular SPA для работы со всеми методами API и Docker Compose для совместного запуска клиента, WebAPI и базы данных.

## Возможности

- загрузка CSV через `multipart/form-data`;
